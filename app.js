...
(function () {
  ...
  const AI_DISCIPLINE = [
    ...
    '4.1 年命宫=该人出生年之**天干**在天盘所落之宫。**年命为甲者，甲不上天盘，须按其本年之甲所遁之仪取宫：甲子→戊、甲戌→己、甲申→庚、甲午→辛、甲辰→壬、甲寅→癸**（如甲子年生人看天盘戊落宫，甲午年生人看天盘辛落宫）。**不得以本盘旬首或值符代之**：纲要「甲干不上天盘（遁于旬首）：日/时干为甲时，以值符落宫论之」只管日干、时干，**不及年命**——本盘旬首随占时而变，年命是人的出生年，不随占时而变。问句中涉及多人（候选人、对手、家人）并给出生年者，各人年命一律照此取宫；只知「甲年」不知年支者，写明无法定宫，不得硬取。（六甲遁六仪为奇门通则，纲要「遁于旬首」即出于此。）',
  ...
  // 年命用同一个 fmt 一并算出即可。
  //
  // **但年命为甲不能走 fmt 里「以值符落宫论」那条**：纲要原文只说「日/时干为甲时」，不及年命；
  // 也不能及——本盘旬首随占时而变，年命是出生年，不随占时而变。年命甲按六甲遁仪取宫
  // （甲子戊、甲戌己…），故界面上甲年须连年支一起选；只有一个「甲」则如实说定不了。
