...
(function (root, factory) {
  ...
  function toPromptBlock(res) {
    ...
    /* ---- 实测偏晚的告诫：本层的锚点几乎清一色指向**未来**某个干支位，
     * 填实/冲实/冲墓按定义都在今日之后，于是断语被系统性地推到几天甚至几个月以后。
     * 案例本导出 2026-09-07 的复盘统计：可判方向的应期错例里，**断得比实际晚者 12 例、
     * 早者 1 例**——猫当天下午找回而断「5–10 日」、摇号当天就成而断「戌月」、
     * 绩效数日内到账而断「子月」、餐饮店 3 月关门而断「10 月」。
     * offset 0 的锚点（「即今日」「即当下这个时辰」）本就在候选里，只是从没被优先看过。 ---- */
    var zeroNow = [];
    (res.units || []).forEach(function (u) {
      ((res.byUnit && res.byUnit[u]) || []).forEach(function (r) {
        if (r.offset === 0) zeroNow.push(r.label + '（' + (r.when || u) + '）');
      });
    });
    L.push('· ⚠ **先看「当下这一档」**：本层锚点几乎清一色指向未来某个干支位——填实、冲实、冲墓'
      + '按定义都在今日之后。实测本仓应期错例中**断得偏晚者 12 例、偏早者 1 例**，'
      + '即「几乎只会晚，不会早」。故问「今天／现在／这两天」的近事，先看有没有当下即可应的锚点：'
      + (zeroNow.length ? '本盘有 ' + zeroNow.slice(0, 6).join('、') : '本盘无 offset 0 的锚点')
      + '。空亡「须待填实／冲实方应」是纲要对**应期**之说，不是「此刻不能应」的禁令；'
      + '要断到填实那一档，须说明为何不是更早。'
      + '（偏晚一说出自〔用户所定·2026-09-07·据案例本 12:1〕，纲要只写了填实冲实之法，未言早晚。）');
