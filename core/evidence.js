...
(function (root, factory) {
  ...
  function toPromptBlock(ev) {
    ...
    if (times.length) {
      ...
      // 实测偏晚的告诫。本层锚点几乎清一色指向未来某个干支位（填实/冲实/冲墓按定义都在今日之后），
      // 断语因此被系统性推后。案例本导出 2026-09-07 的复盘统计：可判方向的应期错例里
      // **断得比实际晚者 12 例、早者 1 例**。offset 0 的锚点本就在候选里，只是从没被优先看过。
      var zeroNow = [];
      (tmi.units || []).forEach(function (u) {
        ((tmi.byUnit || {})[u] || []).forEach(function (r) {
          if (r.offset === 0) zeroNow.push(r.label + '(' + (r.when || u) + ')');
        });
      });
      L.push('  ⚠ **先看「当下这一档」**：本段锚点几乎清一色指向未来——填实、冲实、冲墓按定义都在今日之后。' +
        '实测本仓应期错例中**偏晚 12 例、偏早 1 例**，即「几乎只会晚，不会早」。' +
        '问「今天／现在／这两天」的近事，先看当下即可应的锚点：' +
